...
class BackBone(nn.Module):
    ...
    @torch.no_grad()
    def predict(self, data: Dict, horizon_steps: int | None = None) -> Dict[str, torch.Tensor]:
        """Run WcDT-style trajectory prediction without computing training losses.

        This method is intentionally additive: the original ``forward`` contract is
        preserved for Waymo training, while SAFE_RL can use this method as a frozen
        predictor or feature extractor.
        """
        predicted_feature = data['predicted_feature']
        other_his_pos = data['other_his_pos']
        other_his_traj_delt = data['other_his_traj_delt']
        ...
        if horizon_steps is not None:
            traj = traj[:, :, :, :horizon_steps, :]
        mode_confidence = torch.softmax(confidence, dim=-1)
        max_confidence = torch.max(mode_confidence, dim=-1).values
        uncertainty = 1.0 - max_confidence
        return {
            "future_trajectories": traj,
            "mode_confidence": mode_confidence,
            "scene_latent": scene_feature,
