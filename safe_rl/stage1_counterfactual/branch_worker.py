"""Process-isolated SUMO counterfactual branch execution."""
...
from safe_rl.accvp.planning.candidate_plan import ACCVP_COMMITMENT_PROFILE, apply_branch_command
from safe_rl.stage1_counterfactual.root_context import load_root_context, restore_root_context
...
def _branch_outcome(job: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "candidate_plan_profile": str(root.metadata.get("candidate_plan_profile", ACCVP_COMMITMENT_PROFILE)),
            "accvp_activation_distance_m": float(root.metadata.get("accvp_activation_distance_m", 0.0)),
            "response_horizon_s": float(root.metadata.get("response_horizon_s", config.accvp.response_horizon_s)),
            "viability_horizon_s": float(root.metadata.get("viability_horizon_s", config.accvp.viability_horizon_s)),
            "data_contract": dict(root.metadata.get("data_contract", {})),
            "data_contract_hash": str(root.metadata.get("data_contract_hash", "")),
            ...
            "activation_bin": str(root.metadata.get("activation_bin", root.metadata["deadline_bin"])),
